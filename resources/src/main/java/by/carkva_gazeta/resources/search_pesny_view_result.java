...
public class search_pesny_view_result extends AppCompatActivity implements View.OnTouchListener, Dialog_font_size.Dialog_font_size_Listener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        String resultat = Objects.requireNonNull(getIntent().getExtras()).getString("resultat");
        ArrayList<Menu_list_data> menu_list_data = search_pesny.get_Menu_list_data();
        for (int i = 0; i < menu_list_data.size(); i++) {
            try {
                if (menu_list_data.get(i).data.toLowerCase().contains(Objects.requireNonNull(resultat).toLowerCase())) {
                    try {
                        Field[] fields = R.raw.class.getFields();
                        for (Field field : fields) {
                            if (field.getInt(null) == menu_list_data.get(i).id) {
                                resurs = field.getName();
                                break;
                            }
                        }
                    ...
                    title = menu_list_data.get(i).data;
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        menu.findItem(by.carkva_gazeta.malitounik.R.id.action_dzen_noch).setVisible(false);
